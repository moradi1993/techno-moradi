...
def update_info(request):
     if request.user.is_authenticated:      #authenticated : لاگین
          ...
          if form.is_valid() or shipping_form.is_valid():
               form.save()
               shipping_form.save()
               messages.success(request, 'اطلاعات کاریری ویرایش شد')
...
def helloworld(request):
    all_productas = Product.objects.all()
    return render(request,'index.html' , {'products': all_productas})
...
def update_password(request):
     if request.user.is_authenticated:
          current_user = request.user     # current_user:  کاربر جاری 
          if request.method =='POST':
          ...
          messages.success(request, 'باید اول لاگین بشوید')
          return redirect('home')
...
def signup_user(request):
     ...
     if request.method == "POST":
          ...
          if form.is_valid():
               ...
               user = authenticate(request,username=username,password=password1)   #authenticate = احراز هویت
               login(request,user)
               messages.success(request, (" اکانت شما ساخته شد"))
               return redirect("home")
          else:
...
def user_orders(request):
     if request.user.is_authenticated:
          ...
          messages.success(request, 'بدون احراز هویت دسترسی به این صفحه امکان پذیر نمی باشد')
          return redirect('home')
